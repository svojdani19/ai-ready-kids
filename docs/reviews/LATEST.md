...
only `archived_at`, `archiveClassAction` checks only ownership and the term.
**Two routes reach `students.length === 0 && archived`**: a cohort parked before
enrollment, and an active class whose last student is removed before archiving.
...
Now: *"**No student records to review.** There are no student records on this
roster to review. An administrator has to restore the class before anybody can be
added to it."* No control named, no code mentioned, the absence stated plainly,
the one route out named. Active + empty untouched, form and code guidance
included; archived + non-empty untouched.

**Strictly present-tense, after a second acceptance correction.** The first
version read *"Nobody joined this class before it was archived…"* — a
**historical inference the page cannot support**. `students.length === 0` proves
the roster is empty now and nothing about how it got that way, and for the second
route above it is not merely unsupported but false. It also contradicted this
entry's own text, which named the emptied-then-archived path in the paragraph
above it. The copy now claims nothing about history: not whether anyone joined,
not whether records were removed, not whether retention ran. A guard asserts the
rendered page matches none of six history phrasings, and a test empties a real
cohort through `removeStudentAction` one child at a time before archiving it —
the path the create-then-archive fixture cannot exercise.
...
mutation components absent when archived. **Mutation-checked five ways:**
...
state on an active class fails 1; restoring the retired "Nobody joined this class
before it was archived" fails 3; and a history claim never shipped — "The
students who were on this roster were removed" — also fails 3.
...
roster". Archived + empty at 1280×800 reads exactly *"No student records to review | There
are no student records on this roster to review. An administrator has to restore
the class before anybody can be added to it."* with **no** history claim
anywhere, none of the three retired phrases, no code, "Inactive", the archived
notice, **Sign out as the only button on the page** and no roster table; at
768×1024 the same block renders 686px inside 768 with `overflow: false` — that width's
...
1. **The history guard is a vocabulary, not a rule.** Six phrasings of "how the
   roster came to be empty" are pinned; a seventh would pass — the same denylist
   shape sprints 80, 81 and 83 ended on.
2. **The archived notice still says "the roster below" on an empty class**, and
...
3. **I wrote a known gap without reading the query, and it was backwards.** The
...
4. **Nothing enumerates mutation controls, and there are now two surfaces.**
   ...
   up.
5. **The notice is authored prose in a component**, not in the domain layer
   beside `LAPSED_STAFF_BODY`. Only the title is exported and asserted.
6. **Screen-reader behaviour is asserted structurally, not heard.** Role, label,
