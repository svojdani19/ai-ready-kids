...
export const CERTIFICATION_MODULES: CertificationModule[] = [
  {
    id: "cert-1",
    order: 1,
    // Was "What grades 1 to 5 actually need". True of the age band the product
    // touches, but this title renders in the module list on `/approach`, where a
    // buyer reads it beside the plan copy — and sprint 85's whole point is that
    // no buyer-facing surface attaches 1 to 5 to what is sold. The body's own
    // first sentence names the age range instead, which carries no scope claim.
    title: "What 6 to 11 year olds actually need",
    minutes: 7,
    body: [
      "Six to eleven year olds are not going to be taught their way out of encountering AI. They meet it on a sibling's tablet, in a video game, in a smart speaker in someone else's kitchen, and in a search box that now answers the question instead of helping them learn from it. The question is whether the first request for their address, or the first confident wrong answer, is also the first time they have thought about it.",
      "That is the whole design brief for this program: practice before exposure. The assessed curriculum is 27 authored missions written and reading-levelled for grades 2 to 4, three for each of the nine skills, so a child meets the same decision in three different situations rather than once.",
      "Before any of that there is First Look: six short sessions in two tiers, of which your class runs the three written for its grade. It teaches the three things every core mission assumes. That an AI program works out patterns from a great many examples and uses them to make a guess — a likely next word being one kind of guess, and matching a face or choosing what plays next being others. That it is already inside ordinary tools rather than a robot in a film. And that a person decides when it is used and answers for the result. Without those, a mission about declining a request for a home address can be played by a child who thinks the app asking is alive.",
      "First Look records no skill evidence, deliberately. Answering a comprehension question on the board is not a safety skill in the sense the nine skills mean, so no competency figure moves and no competency total includes it. It does record that a child opened and finished the session, which counts as completed work on the class page, and a First Look run on the board with nobody signed in records nothing about any individual at all.",
      "Every encounter has the same shape: a specific situation, then the decision rule named out loud, then the same rule met again somewhere new. That is a sequence to teach with, not a claim about what a seven year old is capable of. What this age can do is considerable — grade 1 can tell you that a program which fills in the next word has never met their cat, and grade 5 can watch a tool invent a school swimming team and say why. An abstract warning is thin material to work from at any age, and thinner still about a situation nobody has ever met, which is why every mission is a story with a decision in it.",
    ],
    keyPoints: [
