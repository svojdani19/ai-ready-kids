...
export const CLASS_CODE_BOUNDARY = [
  ...
  "Rotate the code whenever it has traveled further than the class. From then on the old code is rejected on the next request that uses it: nobody can join with it, a half-finished join stops, and a browser already signed in with it is asked to rejoin the next time it loads a page. Nothing else about the class changes, and no records are touched.",
