...
/**
 * Sprint 35. The Classes page told administrators that a code could only be
 * regenerated by deleting and recreating the class, on the same page as the
 * button that permanently deletes a roster, mission history, evidence, badges
 * and both check-ins — and the safe rotation had existed since sprint 30. An
 * administrator following the product's own instruction after a code was
 * photographed would have destroyed children's records to do a job the product
 * already knew how to do without touching them.
 */
describe("an administrator can rotate a class code without deleting anything", () => {
  let db5: Db;
  let cleanup5: () => void;
  const admin = { id: DEMO_ADMIN_ID, role: "admin" as const, school_id: DEMO_SCHOOL };

  beforeAll(() => {
    ({ db: db5, cleanup: cleanup5 } = createTestDb());
    // Give the class some history, so "preserved" means something.
    playToEnd(db5, DEMO_STUDENT, MISSIONS[0]);
  });
  afterAll(() => cleanup5());

  // Whole rows, not spot-checked columns: badges are derived from completed
  // attempts rather than stored, so preserving an attempt intact is what
  // preserves a child's badge.
  const childRecords = (table: string) =>
    rows<Record<string, unknown>>(
      db5
        .prepare(
          `SELECT t.* FROM ${table} t JOIN students s ON s.id = t.student_id
            WHERE s.class_id = ? ORDER BY t.id`,
        )
        .all(DEMO_CLASS),
    );

  const linkedRecords = () => ({
    classRow: getClass(db5, DEMO_CLASS)!,
    students: listStudents(db5, DEMO_CLASS),
    assignments: listAssignments(db5, DEMO_CLASS),
    attempts: childRecords("attempts"),
    benchmarks: childRecords("benchmarks"),
    // No certifications: that table is keyed on a staff user, not a student.
    // It is the teacher training module, and a class code never touched it.
  });

  it("changes the join code and nothing else about the class or its children", () => {
    const before = linkedRecords();
    expect(before.attempts.length).toBeGreaterThan(0);
    expect(before.students.length).toBeGreaterThan(0);

    expect(canAdministerClass(admin, before.classRow)).toBe(true);
    const code = rotateJoinCode(db5, DEMO_CLASS);
    ...
    for (const key of Object.keys(before.classRow) as (keyof typeof before.classRow)[]) {
      if (key === "join_code") continue;
      expect(after.classRow[key], key).toEqual(before.classRow[key]);
    }
  ...
  it("leaves a grant issued against the superseded code unusable", () => {
    ...
    // Valid right up to the moment the code changes.
  ...
  it("refuses an administrator from another school", () => {
    const stranger = { role: "admin" as const, school_id: "sch_elsewhere" };
    expect(canAdministerClass(stranger, getClass(db5, DEMO_CLASS)!)).toBe(false);
    // And a teacher at this school is not made an administrator by this rule.
    expect(canAdministerClass({ role: "teacher", school_id: DEMO_SCHOOL }, getClass(db5, DEMO_CLASS)!)).toBe(
      false,
    );
  });

  it("scopes the action through that check and records the rotation without a child's name", () => {
    ...
    expect(body).not.toContain("listStudents");
    // And the guard it goes through is the shared rule, not a second copy.
    const guard = src.slice(src.indexOf("async function ownClass"));
    expect(guard.slice(0, guard.indexOf("\n}"))).toContain("canAdministerClass(user, classroom)");
  });

  it("no longer tells anybody to delete a class to change its code", () => {
    const page = readFileSync(join(process.cwd(), "src/app/admin/classes/page.tsx"), "utf8");
    expect(page).not.toMatch(/regenerated by deleting/);
    expect(page).not.toMatch(/deleting and recreating/i);
    expect(page).toContain("rotateJoinCodeAsAdminAction");
    // The confirmation has to say what survives, because the control sits
    // beside one that destroys all of it.
    expect(page).toContain("mission history, badges and both check-ins are not touched");
  });
});
